-- Fix PL/pgSQL output-column ambiguity in execute_ag_paper_sell_authorized.
-- RETURNS TABLE exposes quantity/ticker/etc as PL/pgSQL variables, so every SQL
-- reference to transaction/FIFO columns must be qualified explicitly.
...
begin
  ...
  select a.* into v_auth from public.ag_sell_execution_authorizations a
  where a.id = p_authorization_id for update;
  ...
  select d.* into v_decision from public.investment_decisions d where d.id = v_auth.decision_id for update;
  ...
  select e.* into v_era from public.portfolio_strategy_eras e where e.id = v_auth.strategy_era_id for update;
  ...
  select p.* into v_portfolio from public.portfolios p where p.id = v_auth.portfolio_id for update;
  ...
  for v_row in
    select tx.id, tx.transaction_type, tx.quantity as tx_quantity, tx.price_per_share as tx_price_per_share,
           coalesce(tx.fees,0) as tx_fees, tx.transaction_date, tx.created_at
  ...
  loop
    if v_row.transaction_type = 'buy' then
      ...
      values (v_row.tx_quantity, ((v_row.tx_quantity * v_row.tx_price_per_share) + v_row.tx_fees) / v_row.tx_quantity);
    elsif v_row.transaction_type = 'sell' then
      v_remaining := v_row.tx_quantity;
      for v_lot_seq in select l.seq from pg_temp.ag_fifo_lots l where l.remaining_quantity > 0 order by l.seq loop
        exit when v_remaining <= 0;
        select least(l.remaining_quantity, v_remaining) into v_take from pg_temp.ag_fifo_lots l where l.seq = v_lot_seq;
        update pg_temp.ag_fifo_lots l set remaining_quantity = l.remaining_quantity - v_take where l.seq = v_lot_seq;
  ...
  select coalesce(sum(l.remaining_quantity),0) into v_owned from pg_temp.ag_fifo_lots l;
  if v_auth.quantity > v_owned then raise exception 'AG SELL quantity exceeds era holdings'; end if;

  v_remaining := v_auth.quantity;
  v_basis := 0;
  for v_row in select l.seq as lot_seq, l.remaining_quantity as lot_quantity, l.unit_cost as lot_unit_cost from pg_temp.ag_fifo_lots l where l.remaining_quantity > 0 order by l.seq loop
    exit when v_remaining <= 0;
    v_take := least(v_row.lot_quantity, v_remaining);
    v_basis := v_basis + (v_take * v_row.lot_unit_cost);
    v_remaining := v_remaining - v_take;
  end loop;
  ...
  insert into public.transactions(user_id, portfolio_id, transaction_type, ticker, quantity, price_per_share, gross_amount, fees, transaction_date, notes, cost_basis, realized_gain_loss, lot_method)
  values (v_auth.user_id, v_auth.portfolio_id, 'sell', upper(v_auth.ticker), v_auth.quantity, v_auth.price_per_share, v_gross, v_auth.fees, now(), 'Authorized Accelerated Growth paper SELL for decision ' || v_auth.decision_id, round(v_basis,2), round(v_net-v_basis,2), 'fifo')
  returning public.transactions.id into v_transaction_id;

  update public.investment_decisions d set transaction_id = v_transaction_id, status = 'executed'
  ...
  update public.ag_sell_execution_authorizations a set consumed_at = now(), transaction_id = v_transaction_id
  ...
  return query select v_transaction_id, v_auth.decision_id, upper(v_auth.ticker), v_auth.quantity, v_auth.price_per_share,
    v_gross, v_auth.fees, v_net, round(v_basis,2), round(v_net-v_basis,2), v_owned, v_owned-v_auth.quantity;
end;
$$;
