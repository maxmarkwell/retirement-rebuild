-- Retirement Rebuild
-- Harden atomic Accelerated Growth paper BUY execution against direct RPC
-- callers bypassing application sizing. The database enforces the immutable
-- v1 execution invariants; live marked-value/theme/thesis gates remain
-- application responsibilities and must pass before this RPC is called.
...
declare
  ...
  v_ticker_buy_gross numeric := 0;
  v_ticker_sell_gross numeric := 0;
  v_ticker_net_cost numeric := 0;
...
begin
  ...
  select
    ...
    coalesce(sum(case when transaction_type = 'buy' then gross_amount + fees else 0 end), 0),
    coalesce(sum(case when transaction_type = 'sell' then gross_amount - fees else 0 end), 0)
    into v_existing_qty, v_ticker_buy_gross, v_ticker_sell_gross
    from public.transactions
   where portfolio_id = v_portfolio.id
     and upper(ticker) = upper(v_decision.ticker)
     and created_at >= v_era.inception_at;

  v_ticker_net_cost := v_ticker_buy_gross - v_ticker_sell_gross;

  -- v1 ADDs are intentionally disabled until explicit reassessment is persisted.
  ...
  -- Starter target is $5. Three-decimal brokerage precision may overshoot it,
  -- but the immutable hard position cap may never be breached.
